    ...
    implements DeviceCmdHandler
{
    ...
    **/
    public boolean deviceSupportsCommands(Device dev)
    {
        ...
        if ((acct == null) || !acct.getSmsEnabled()) {
            Print.logWarn("Account SMS is disabled: " + dev.getAccountID());
            return false;
        }
    ...
    **/
        ...
        throws IOException
    {
        ...
        if (cmdCount == 0) {
            Print.logWarn("No available commands for Device: " + selDevID);
    ...
    **/
    private static String GetResponseMessage(RTProperties resp, I18N i18n)
    {
        if (resp == null) {
            ...
            DCServerFactory.ResultCode rsID = DCServerFactory.GetResultCode(result,null);
            if (rsID == null) {
                Print.logWarn("Unrecognized ResultCode, assuming SUCCESS: " + result);
    ...
    **/
    private RTProperties sendCommand(Device device, DCServerConfig.Command command, String cmdArgs[])
    {
        ...
        /* supported device? */
        if (!this.deviceSupportsCommands(device)) {
            return CommandPacketHandler.setResult(rtp, DCServerFactory.ResultCode.NOT_AUTHORIZED);
        }

        /* get command */
        String smsCmd = command.getCommandString(device, cmdArgs);
