            ...
            public void write(PrintWriter out) throws IOException {
              ...
              //String menuURL    = EncodeMakeURL(reqState,RequestProperties.TRACK_BASE_URI(),PAGE_MENU_TOP);
                ...
                if (_listAccounts) {
                    ...
                    if (showAllowWebService && isSysAdmin) {
                        // -- "Allow Web Service":
                        boolean isAllowed = Account.GetAllowWebService(_selAccount); // Account.alwaysAllowWebService
                        // --
                        ComboOption allowOpt;
                        ComboMap    allowMap;
                        String      allowDesc;
                        if (Account.IsAlwaysAllowWebService()) {
                            allowOpt  = ComboOption.getYesNoOption(locale, isAllowed);
                            allowMap  = new ComboMap();
                            if (isAllowed) {
                                // -- should ALWAYS be "true"
                                allowMap.add(ComboOption.BOOLEAN_NAME_TRUE , ComboOption.getYesNoText(locale, true ));
                            } else {
                                // -- this should never occur (should ALWAYS be "true")
                                allowMap.add(ComboOption.BOOLEAN_NAME_FALSE, ComboOption.getYesNoText(locale, false));
                            }
                            allowDesc = i18n.getString("SysAdminAccounts.alwaysAllowWebService","(always allowed)");
                        } else {
                            allowOpt  = ComboOption.getYesNoOption(locale, isAllowed);
                            allowMap  = ComboMap.getYesNoMap(locale);
                            allowDesc = i18n.getString("SysAdminAccounts.accountAllowWebService","(specified per account)");
                        }
                        out.println(FormRow_ComboBox(PARM_ACCT_WEB_SERVICE, editSysAdmin, i18n.getString("SysAdminAccounts.allowWebService","Allow Web Service")+":", allowOpt, allowMap, "", -1, allowDesc));
                    }
