    ...
    extends ReportData
{
    ...
    **/
        ...
        throws ReportException
    {
        ...
        /* check EventData for InnoDB */
        DBFactory<EventData> edFact = EventData.getFactory();
        if (edFact.isMySQLInnoDB() && !edFact.getAllowInnoDBCOUNT()) {
            // -- this report uses "COUNT(*)" to count records. If EventData is an InnoDB 
            // -  table, and "COUNT(*)" is disallowed, then this report will not provide 
            // -  any count information.
            Print.logWarn("EventData table is InnoDB, and 'COUNT(*)' is disallowed.");
            Print.logWarn("Set the following property to enable 'COUNT(*)' with 'WHERE' for InnoDB:");
            Print.logWarn("  " + DBConfig.PROP_EventData_allowInnoDBCountWithWhere + "=true");
        }
