...
public class TemperatureSet
{
    ...
    **/
    public static String CreateGoogleDataTableJavaScript(boolean F, TemperatureSet... TSList)
    {
        ...
        for (int d = 0; d < TSList.length; d++) {
            TemperatureSet TS = TSList[d];
            String id    = JSON.escapeJSON("temp"+(d+1));
            String label = JSON.escapeJSON(TS.getLabel("Temp-"+(d+1)));
            String type  = JSON.escapeJSON("number");
    ...
    private static String CreateGoogleDataTableJSON(boolean F, TemperatureSet... TSList)
    {
        ...
        if (!ListTools.isEmpty(TSList)) {
            for (int d = 0; d < TSList.length; d++) {
                TemperatureSet TS = TSList[d];
                String id    = "temp" + (d+1);
                String label = TS.getLabel("Temp-"+(d+1));
                String type  = "number";
                JSON._Object col_temp = (new JSON._Object()).setFormatIndent(false);
                col_temp.addKeyValue("id"   , id);
                col_temp.addKeyValue("label", label);
                col_temp.addKeyValue("type" , type);
    ...
    *** Constructor
    **/
    public TemperatureSet(String label)
    {
        this();
    ...
    *** Constructor
    **/
    public TemperatureSet(String label, Temperature TList[])
    {
        this(label);
    ...
    *** Constructor
    **/
    public TemperatureSet(String label, Collection<Temperature> TList)
    {
        this(label);
        if (!ListTools.isEmpty(TList)) {
            for (Temperature T : TList) {
    ...
    **/
    public String getLabel(String dft)
    {
        return !StringTools.isBlank(this.label)? this.label : dft;
    }
