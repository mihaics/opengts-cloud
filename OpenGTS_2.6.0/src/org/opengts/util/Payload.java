...
public class Payload
{
    ...
    *** @param crc     The CRC algorithm to use
    ...
    **/
    public long calcChecksum(Checksum.CRC crc, int bOfs, int bLen)
    {
        ...
        return Checksum.calcChecksum(crc, b, ofs, len);
    }
    ...
    **/
    ...
    {
        Checksum.CRC crc = Checksum.CRC.CCITT; // 16-bit
        return (int)this.calcChecksum(crc, bOfs, bLen);
    }
    ...
    *** @param bOfs  The offset
    *** @param bLen  The length
    *** @return The calculated CCITT:Xmodem CRC
    **/
    public int calcCrcXmodem(int bOfs, int bLen)
    {
        Checksum.CRC crc = Checksum.CRC.XMODEM; // 16-bit
        return (int)this.calcChecksum(crc, bOfs, bLen);
    }
    ...
    **/
    ...
    {
        Checksum.CRC crc = Checksum.CRC.CRC16; // 16-bit
        return (int)this.calcChecksum(crc, bOfs, bLen);
    }
    ...
    *** @param bOfs  The offset
    *** @param bLen  The length
    *** @return The calculated Modbus CRC-16
    **/
    public int calcCrc16Modbus(int bOfs, int bLen)
    {
        Checksum.CRC crc = Checksum.CRC.MODBUS; // 16-bit
        return (int)this.calcChecksum(crc, bOfs, bLen);
    }
    ...
    **/
    ...
    {
        Checksum.CRC crc = Checksum.CRC.SUM8; // 8-bit
        return (int)this.calcChecksum(Checksum.CRC.SUM8, bOfs, bLen);
    }
    ...
    **/
    public static long extendSignBit(long val, int bitLen)
    {
        ...
        /* create value-bit-mask and negative-bit-mask */
        long bitMask;
        long negBit;
        if (bitLen <= 0) {
            // -- invalid bit-length, assume "0"
            return 0L;
        } else
        if (bitLen >= 64) {
            // -- maximum bit-length
            bitMask = 0xFFFFFFFFFFFFFFFFL;
            negBit  = 0x8000000000000000L;
        } else {
            // -- specified bit-length
            bitMask = (1L << bitLen) - 1L;
            negBit  =  1L << (bitLen - 1);
        }

        /* return sign extended value */
        if ((val & negBit) != 0L) {
            // -- negative (set MSB to FF..FF)
            return val | ~bitMask;
        } else {
            // -- positive (clear MSB)
            return val & bitMask;
    ...
    *** @param data      The byte array into which the value is encoded
    *** @param ofs       The offset into <code>data</code> to start encoding
    *** @param len       The number of bytes to encode
    *** @param bigEndian True if the bytes are to be big-endian ordered, false if little-endian
    *** @param val       The value to encode
    *** @return The number of bytes written to <code>data</data>.
    **/
    ...
    **/
    public static byte[] encodeLong(long val, int len, boolean bigEndian)
    {
        ...
        int n = Payload.encodeLong(data, 0, len, bigEndian, val);
        return data;
    }
